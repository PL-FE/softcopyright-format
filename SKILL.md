...
在开始生成正式版软著材料前，若以下信息缺失，**必须主动先向用户索取**，不要直接凭空假设：

1. **软件名称**
   - 申请表、说明书、源代码页眉中的名称必须一致
2. **版本号**
   - 例如 `V1.0`、`V1.0.0`
   - 申请表、说明书、源代码页眉中的版本号必须一致
3. **产品/软件描述**
   - 至少需要软件用途、主要功能、适用场景
   - 如果用户提供的是推广文案，需要转写为更像软著说明书的正式表述
4. **截图素材**
   - 若用户希望说明书带界面截图，必须先确认截图目录或截图文件
   - 未提供时要明确提示“说明书将不包含截图”
5. **源码范围**
   - 要确认是否只取业务目录，是否排除 `ios`、`android`、`node_modules` 等目录
6. **项目路径**
   - 若用户未明确，应先确认当前目录是否就是目标项目

### 主动追问规则

若缺少上述任一关键材料，应优先向用户确认，常见追问包括：
- 软件名称和版本号分别是什么？
- 说明书里要不要带截图？截图目录在哪里？
- 软著源代码具体取哪些目录？是否排除 `ios/android`？
- 有没有可用于说明书的产品介绍、功能描述或 PRD 文案？

### 允许自动推断，但必须告知用户的项目

以下信息可以先自动识别，但要在生成前明确告诉用户你采用了什么值：
- 从 `app.json`、`package.json` 自动读取的软件名称
- 从配置文件自动读取的版本号
- 自动发现的截图目录
- 自动识别的源码目录和排除目录

若用户后续指定了这些值，必须以用户指定为准。

## 正式版生成前的拦截规则

若用户要求生成“可直接提交”的正式版软著材料，而以下任一信息仍未确认：
- 软件名称
- 版本号
- 产品/软件描述
- 是否需要截图及截图来源
...
则必须先停止生成，改为向用户补齐信息；不要直接输出正式版材料。
...
# 方式3: 使用快捷命令（最佳体验）
~/.claude/skills/softcopyright-format/softcopyright-generate --project . --auto-pdf
...
```bash
node ~/.claude/skills/softcopyright-format/scripts/scanner.js <项目路径>
```
...
```bash
node ~/.claude/skills/softcopyright-format/scripts/doc-generator.js \
  --type manual \
  --input <项目分析JSON> \
  --template <模板文件> \
  --output <输出PDF路径>
...
```bash
node ~/.claude/skills/softcopyright-format/scripts/source-exporter.js \
  --input <源码目录> \
  --output <输出PDF路径> \
  --pages 60 \
  --lines-per-page 50
